<template>
	<view>
		<cell class = "item "v-for="(item,index) in hisList" :key="item.id" @click="goDetail(item)" >
...
<script>
	export default {
		...
		onLoad() {
			//初始化页面时获取所有历史记录
			uni.$u.http.get("/news/getAllHistory/" + String(this.currentUser.user.id) + '/' + String(this.pageNo), {custom : {auth: true}}).then(res=> {
				// console.log(res.histories)
				this.totalPage = res.pageNum ;
				if (this.pageNo == 1) this.hisList = res.histories ;
				else {
					for (var i = 0 ; i < res.histories.length ; i++) {
						this.hisList.push(res.histories[i]);
					}
				}
			})
			.catch(err => {
